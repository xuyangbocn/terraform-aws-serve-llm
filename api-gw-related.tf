...
resource "aws_apigatewayv2_vpc_link" "llm" {
  ...
  security_group_ids = [one(aws_security_group.llm_vpc_link_sg[*].id)]
...
resource "aws_vpc_security_group_egress_rule" "llm_vpc_link_egress_1" {
  ...
  security_group_id = one(aws_security_group.llm_vpc_link_sg[*].id)
...
resource "aws_apigatewayv2_route" "root_post" {
  ...
  route_key = "POST /{llmServerApiPath+}"
  target    = "integrations/${one(aws_apigatewayv2_integration.root_post[*].id)}"
}
...
resource "aws_apigatewayv2_integration" "root_post" {
  ...
  connection_id   = one(aws_apigatewayv2_vpc_link.llm[*].id)
...
resource "aws_apigatewayv2_route" "root_get" {
  ...
  target    = "integrations/${one(aws_apigatewayv2_integration.root_get[*].id)}"
}
...
resource "aws_apigatewayv2_route" "root_delete" {
  ...
  target    = "integrations/${one(aws_apigatewayv2_integration.root_delete[*].id)}"
}
