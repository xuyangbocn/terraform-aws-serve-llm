...
resource "aws_vpc_security_group_ingress_rule" "llm_ec2_ingress_app_port" {
  ...
  security_group_id = aws_security_group.llm_ec2_sg.id
  description       = "Traffic over Application (e.g. vllm or ollama) Port from VPC"
...
resource "aws_ssm_document" "pull_models" {
  for_each = toset(["vllm", "ollama"])

  name          = "${var.prefix}-${each.key}-pull-models"
  document_type = "Command"

  content = file("${path.module}/aws-ssm-document/${each.key}-pull-models.json")
}
